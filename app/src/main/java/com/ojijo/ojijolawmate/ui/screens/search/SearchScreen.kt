package com.ojijo.ojijolawmate.ui.screens.search
...
fun SearchScreen(navController: NavController) {
    ...
    val filteredLaws = allLaws.filter {
        it.section.contains(query.text, ignoreCase = true) ||
    ...
    Column(
        ...
        LazyColumn(
            ...
            items(filteredLaws) { law ->
                LawSearchCard(law = law) {
                    // Navigate to law detail with section as argument
                    navController.navigate("law_detail_screen/${law.section}")
                }
            }
        }
    }
...
@Composable
fun LawSearchCard(law: Law, onClick: () -> Unit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() },
